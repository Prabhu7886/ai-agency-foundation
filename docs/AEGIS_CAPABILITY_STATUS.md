...
| Aegis Hub | Encrypted owner-controlled identity profile, global Executive Partner popup, locked portrait and full-body assets, operational local single-frame screen analysis, browser motion states, incognito metadata policy, interruptible local voice states, Academy course plans, verified learning materials, assessments, completion gates, visible preference memory, and owner-reviewed response training candidates | `gemma3:4b` is installed and locally verified; screen access is permissioned each session, audio is never requested, raw frames are never retained, and continuous watching remains disabled |
| Agent Fleet | Authenticated local bridges for independent Commerce and Career runtimes; separate agent-data and supervision-transport keys; actual runtime health, encrypted task timing/outcomes, 30-second monitoring, incident reports, isolated containment drills, capability-level containment, approval-gated recovery, and hashed learning deployment/rollback | Monitoring runs while Aegis is online; private payloads never cross the bridge; agents continue independently when Aegis is offline |
...
| Interface system | Decision-first Aegis Executive Home, Chat/Research/Code/Learn AI Workspace, Mission Control, a global Executive Partner popup, and cohesive specialized Commerce and Career workspaces with responsive interaction states and selection-based Design Edit Mode | A selected-region request is rewritten and sent to the existing Codex approval path; Aegis cannot silently or autonomously rewrite its interface |
...
- Always-listening/hotword voice, offline motion rendering, repeatable phoneme lip sync, and long-form duplex conversation.
...
- Continuous or background laptop observation. The current companion analyzes one owner-triggered frame at a time and never records.
