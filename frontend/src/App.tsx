      ...
      <main className="main-stage">
        ...
        <section className="workspace-stage">
          ...
          {activeWorkspace === "aegis-hub" && <AegisHub
            ...
            onTrainingDecision={(id, status) => mutate(() => decideTrainingCandidate(id, status), `Training candidate ${status}`)}
...
function AIWorkspace({ project, conversations, onRefresh }: { project: Project | null; conversations: Conversation[]; onRefresh: () => Promise<void> }) {
  ...
  const rateAnswer = async (item: ConversationMessage, rating: "helpful" | "too_generic" | "incorrect" | "missed_intent") => {
    if (item.ephemeral || item.id.startsWith("pending-")) return;
    let correction = "";
    if (rating !== "helpful") {
      correction = window.prompt(
        "What should Aegis do differently? This stays encrypted locally and requires your review before it becomes a preference.",
        "",
      )?.trim() ?? "";
    }
    await submitResponseFeedback(item.id, rating, correction);
    setFeedbackByMessage((current) => ({ ...current, [item.id]: rating }));
    setStreamStatus("Feedback saved as a local training candidate for your review");
    await onRefresh();
  };
...
function AegisHub({ data, project, onCreateCourse, onUpdateCourse, onAddMaterial, onAddAssessment, onCreateMemory, onMemoryDecision, onTrainingDecision, onUpdateIdentity, onStartSession, onAddSessionNote, onFinishSession }: {
  data: Bootstrap;
  project: Project | null;
  ...
  onTrainingDecision: (id: string, status: "approved" | "rejected") => Promise<void>;
  ...
  const [screenQuestion, setScreenQuestion] = useState("What is visible, and what should I pay attention to?");
  ...
  const [avatarMotion, setAvatarMotion] = useState<"idle" | "listening" | "speaking">("idle");
  ...
  const captureScreenFrame = () => {
    ...
    if (!video || !video.videoWidth || !video.videoHeight) throw new Error("Screen preview is not ready yet");
    const margin = Math.min(video.videoWidth, video.videoHeight) * (cropMargin / 100);
    const sourceWidth = Math.max(1, video.videoWidth - margin * 2);
    const sourceHeight = Math.max(1, video.videoHeight - margin * 2);
    ...
    context.drawImage(video, margin, margin, sourceWidth, sourceHeight, 0, 0, canvas.width, canvas.height);
  ...
  const analyzeCapturedFrame = async () => {
    if (!activeSession || !screenFrame) return;
    setScreenAnalyzing(true);
    setScreenError("");
    try {
      const result = await analyzeScreenFrame(activeSession.id, screenFrame, screenQuestion);
      setScreenAnalysis(result.analysis);
      setScreenFrame(null);
    } catch (reason) {
      setScreenError(reason instanceof Error ? reason.message : "Local screen analysis failed safely");
    } finally {
      setScreenAnalyzing(false);
    }
  };

  const saveScreenAnalysis = async () => {
    if (!activeSession || !screenAnalysis || activeSession.privacy_mode === "private_incognito") return;
    await onAddSessionNote(activeSession.id, { content: `Screen analysis: ${screenAnalysis}`, learning_candidate: false });
    setScreenAnalysis("");
  };
  ...
  return <div className="single-workspace hub-workspace">
    ...
    {tab === "identity" && <div className="identity-workspace">
      ...
      <section className="panel avatar-motion-panel"><PanelHeader icon={<Sparkles size={17} />} title="Living avatar preview" action={<StatusPill tone="safe">Local browser motion</StatusPill>} /><div className={`avatar-motion-stage avatar-motion-${avatarMotion}`}><div className="avatar-motion-halo" /><img src="/aegis-full-body-v1.png" alt={`Aegis full-body ${avatarMotion} preview`} /><div className="avatar-speech-pulse"><i /><i /><i /></div></div><div className="segmented-tabs avatar-motion-controls">{(["idle", "listening", "speaking"] as const).map((mode) => <button key={mode} className={avatarMotion === mode ? "active" : ""} onClick={() => setAvatarMotion(mode)}>{mode}</button>)}</div><p className="muted">This is a real-time presentation preview, not rendered lip sync. Repeatable mouth animation needs a layered face asset or a separately tested offline renderer.</p></section>
    ...
    {tab === "companion" && <div className="companion-layout">
      <section className="panel companion-stage"><PanelHeader icon={<AppWindow size={17} />} title="Laptop companion" action={<StatusPill tone={activeSession ? "safe" : "neutral"}>{activeSession ? "Session active" : "Permission each time"}</StatusPill>} />{screenPreview ? <video ref={screenVideo} muted playsInline className="screen-preview" /> : <div className="screen-preview screen-preview--empty"><AppWindow size={38} /><strong>No screen is shared</strong><p>A browser permission prompt is required every session.</p></div>}<div className="privacy-facts"><span><LockKeyhole size={13} /> Loopback only</span><span><Square size={13} /> No recording</span><span><ScanEye size={13} /> One frame per click</span></div>{screenPreview && activeSession && <div className="screen-analysis-controls"><label>Crop private edges · {cropMargin}%<input type="range" min="0" max="30" step="5" value={cropMargin} onChange={(event) => setCropMargin(Number(event.target.value))} /></label><button className="secondary-button" onClick={captureScreenFrame}>Capture one frame</button></div>}{screenFrame && <div className="captured-frame"><img src={screenFrame} alt="Captured screen frame awaiting local analysis" /><label>Ask about this frame<textarea value={screenQuestion} onChange={(event) => setScreenQuestion(event.target.value)} /></label><div className="companion-actions"><button onClick={() => setScreenFrame(null)}>Discard</button><button className="primary-button" disabled={screenAnalyzing || !screenQuestion.trim()} onClick={() => void analyzeCapturedFrame()}>{screenAnalyzing ? "Analyzing locally…" : "Analyze once"}</button></div></div>}{screenError && <div className="incognito-notice"><ShieldCheck size={18} /><p>{screenError}</p></div>}{screenAnalysis && <div className="screen-analysis-result"><StatusPill tone="safe">Raw frame discarded</StatusPill><p>{screenAnalysis}</p>{activeSession?.privacy_mode === "standard" ? <button className="secondary-button" onClick={() => void saveScreenAnalysis()}>Save analysis as encrypted note</button> : <small>Incognito policy blocks saving this analysis.</small>}</div>}{screenPreview && <button className="secondary-button" onClick={stopScreenPreview}>Stop screen preview</button>}<small>{data.digital_identity.screen_companion.available ? `${data.digital_identity.screen_companion.model} ready` : data.digital_identity.screen_companion.error ?? "Local vision model is not installed"}. Passwords, private messages, and financial details should be hidden before capture.</small></section>
      {!activeSession ? <form className="panel companion-form" onSubmit={submitSession}><PanelHeader icon={<BrainCircuit size={17} />} title="Start a guided session" /><label>Session type<select value={sessionDraft.session_type} onChange={(event) => setSessionDraft({ ...sessionDraft, session_type: event.target.value })}><option value="study">Study</option><option value="task">Task</option><option value="research">Research</option><option value="creative">Creative</option></select></label><label>Privacy<select value={sessionDraft.privacy_mode} onChange={(event) => setSessionDraft({ ...sessionDraft, privacy_mode: event.target.value })}><option value="standard">Standard · encrypted notes</option><option value="private_incognito">Private incognito · metadata only</option></select></label><label>Screen access<select value={sessionDraft.screen_access} onChange={(event) => setSessionDraft({ ...sessionDraft, screen_access: event.target.value })}><option value="local_preview">Local preview + owner-triggered analysis</option><option value="none">No screen</option></select></label><label>Purpose<textarea required value={sessionDraft.purpose} onChange={(event) => setSessionDraft({ ...sessionDraft, purpose: event.target.value })} placeholder="What are we learning or working through?" /></label><button className="primary-button">Start session</button><small>Screen access is consented per session. Aegis never watches continuously; only a frame you explicitly capture is analyzed, and raw pixels are discarded.</small></form> : <section className="panel companion-active"><PanelHeader icon={<BrainCircuit size={17} />} title={`${activeSession.session_type} session`} action={<StatusPill tone={activeSession.privacy_mode === "private_incognito" ? "warning" : "safe"}>{activeSession.privacy_mode.replaceAll("_", " ")}</StatusPill>} /><p>{activeSession.purpose || "Purpose hidden by incognito policy"}</p>{activeSession.privacy_mode === "standard" ? <form onSubmit={submitSessionNote}><textarea value={sessionNote} onChange={(event) => setSessionNote(event.target.value)} placeholder="Record what Aegis should understand from this step…" /><label className="check-label"><input type="checkbox" checked={learningCandidate} onChange={(event) => setLearningCandidate(event.target.checked)} /> Propose this note as a learning candidate</label><button className="secondary-button" disabled={!sessionNote.trim()}>Save local note</button></form> : <div className="incognito-notice"><LockKeyhole size={18} /><p>Notes, summaries, and learning candidates are blocked for this session. Only minimal audit metadata remains.</p></div>}<textarea value={sessionSummary} onChange={(event) => setSessionSummary(event.target.value)} disabled={activeSession.privacy_mode === "private_incognito"} placeholder="Optional closing summary" /><div className="companion-actions"><button onClick={() => void finishSession("aborted")}>Abort</button><button className="primary-button" onClick={() => void finishSession("completed")}>Complete session</button></div></section>}
